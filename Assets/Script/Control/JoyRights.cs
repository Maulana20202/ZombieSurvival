...
public class JoyRights : MonoBehaviour
{
    ...
    public void Move(BaseEventData data)
    {
        ...
        {
            float joystickXBaseMovement = backgroundImage.transform.position.x - (stick.transform.position.x - pointerData.position.x);
            float joystickYBaseMovement = backgroundImage.transform.position.y - (stick.transform.position.y - pointerData.position.y);
            backgroundImage.transform.position = new Vector2(joystickXBaseMovement, joystickYBaseMovement);
        }
    }
    ...
    // Methods below are used if sticky joystick option is enabled
    ...
    {
        weaponEvent.onWeaponShoot?.Invoke();
        Debug.Log("Shoot");
    }

    public void WeaponRelease()
    {
        weaponEvent.onWeaponRelease?.Invoke();
    }
}
